...
COLUMN_MAP = {
    "tipo_documento": {
        "arl":   ("coalesce", ["AFATDO", "EMPTDO"]),
        "salud": ("single",   ["IDT_NCODE"]),
        "pyc":   ("single",   ["cod_tipo_doc"]),
    },
    "num_documento": {
        "arl":   ("coalesce", ["AFANIT", "EMPIDE"]),
        "salud": ("coalesce", ["PER_CIDENTIFICATIONNUMBER", "INS_CIDTENTIFICATIONNUMBER"]),
        "pyc":   ("coalesce", ["nro_nit", "nro_doc"]),
    },
    "primer_nombre": {
        "arl":   ("single", ["EMPNO1"]),
        "salud": ("single", ["PER_CFIRSTNAME"]),
        "pyc":   ("single", ["txt_nombre"]),
    },
    "primer_apellido": {
        "arl":   ("single", ["EMPAP1"]),
        "salud": ("single", ["PER_CLASTNAME"]),
        "pyc":   ("single", ["txt_apellido1"]),
    },
    "email": {
        "arl":   ("coalesce", ["AFAEML", "EMPEML"]),
        "salud": ("coalesce", ["PER_CEMAIL", "PER_CMAIL"]),
        "pyc":   ("single",   ["txt_direccion"]),
    },
    "celular": {
        "arl":   ("coalesce", ["AFATE1", "AFATE2", "EMPTEL"]),
        "salud": ("single",   ["PER_CMOBILEPHONE"]),
        "pyc":   ("single",   ["txt_telefono"]),
    },
    "direccion_residencia": {
        "arl":   ("coalesce", ["AFADIR", "EMPDIR"]),
        "salud": ("single",   ["ADD_CADDRESS"]),
        "pyc":   ("single",   ["txt_direccion"]),
    },
    "ciudad_residencia": {
        "arl":   ("coalesce", ["AFACIU", "EMPCIU"]),
        "salud": ("single",   ["CIT_NCODE"]),
        "pyc":   ("single",   ["txt_desc"]),
    },
    "fecha_nacimiento": {
        "arl":   ("single", ["EMPFNA"]),
        "salud": ("single", ["PER_DBIRTHDATE"]),
        "pyc":   ("single", ["fec_nac"]),
    },
    "genero": {
        "arl":   ("single", ["EMPSEX"]),
        "salud": ("single", ["PER_CGENDER"]),
        "pyc":   ("single", ["txt_sexo"]),
    },
    "eps_actual": {
        "arl":   None,
    ...
    "fecha_expedicion_documento": {
        "arl":   None,
        "salud": ("single", ["PER_BAUTHPERSONALINFO"]),
        "pyc":   None,
    },
    "atdp": {
        "arl":   ("coalesce", ["AFNFU3", "EMPEPA"]),
        "salud": ("coalesce", ["PER_BEMAIL_SEND", "PER_BSMSEND", "INS_BEMAIL_SEND", "INS_BSMS_SEND"]),
        "pyc":   ("single",   ["sn_aut"]),
    },
    "preferencia_contacto": {
        "arl":   None,
        "salud": ("coalesce", ["PER_NCODE", "INS_NCODE"]),
        "pyc":   None,
    },
    "atdp_comercial": {
        "arl":   None,
    ...
    "estado_civil": {
        ...
        "pyc":   ("single", ["cod_est_civil"]),
    },
    "actividad_economica": {
        "arl":   None,
        "salud": ("single", ["COU_NCODE"]),
        "pyc":   ("single", ["txt_descr"]),
    },
    "departamento": {
        "arl":   ("single", ["CIUCDP"]),
    ...
    "pais": {
        "arl":   None,
        "salud": ("single", ["ACO_CONTRACT_CODE"]),
        "pyc":   ("single", ["txt_desc"]),
    },
    "rol": {
        "arl":   None,
        "salud": None,
        "pyc":   ("coalesce", ["cod_aseg", "id_persona"]),
    },
    "tipo_contrato": {
        "arl":   ("single", ["TIPO_CONTRATO"]),
        "salud": None,
        "pyc":   None,
    },
    "plan": {
    ...
    "contrato": {
        "arl":   ("single", ["AFNAFI"]),
        "salud": None,
        "pyc":   ("concat", ["cod_suc", "cod_ramo", "nro_pol"]),
    },
    "asesor": {
        "arl":   ("single", ["ICOCLA"]),
        "salud": ("single", ["CTI_NCODE"]),
        "pyc":   ("single", ["cod_agente"]),
    },
    "estado": {
        "arl":   None,
        "salud": None,
